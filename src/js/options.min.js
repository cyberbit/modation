var lastPage,save=!0,tracks=[],pages=[],trackInfo=[],optionDefaults={desktop_notifs:!0,sticky_sidebars:!0,group_mods:!0,player_downloads:!0};$(function(){initOptions();initTracks();initWatchlist();initPages();$(window).on("hashchange",function(){parseHash()});selectMenu("tab-community","menu-sticky-sidebars");login()});function initOptions(){$("#check-login").click(login);$(".option").change(save_options);console.debug("options initialized")}
function initTracks(){$("#track-filter").hide();$("#download-tracks").click(getTracks);console.debug("track manager initialized")}function initWatchlist(){$("#check-watchlist").click(function(){$("#watchlist-container .wItem").addClass("load");check_watchlist(!0,refresh_watchlist)});console.debug("watchlist initialized")}
function initPages(){$("header .sub a").click(function(){$(this).attr("data-tab");location.hash="#"+$(this).attr("data-hash")});$(".menu a").click(function(){var a=$(this).parents(".main-wrapper").attr("id"),b=$(this).attr("data-menu");selectMenu(a,b,function(a){"menu-watchlist"==a&&refresh_watchlist()})});console.debug("pages initialized")}function _factory(a){return $(".factory ."+a).clone()}
function parseHash(){""===location.hash&&(location.hash="#general");"#general"===location.hash&&selectTab("tab-general");"#notifs"===location.hash&&selectTab("tab-notifs");"#community"===location.hash&&selectTab("tab-community");"#tracks"===location.hash&&selectTab("tab-tracks");"#about"===location.hash&&selectTab("tab-about")}
...
function save_options(){crapi.clone(function(a){var b=$("#email").val(),c=a[b];save&&$(".option").each(function(){var a=$(this).attr("id");c[a]=$(this).is(":checked")});crapi.update(b,c,function(){status("Settings saved.")})})}function progressStatus(a,b){$("#"+a+" #progressStatus").text(b)}function progress(a,b){$("#"+a+" #progress").progressbar("value",b)}function status(a){a="undefined"==typeof a?"Something happened.":a;console.log("status");showNotificationBar(a,1300,"#15842f","white")}
function error(a){showNotificationBar("undefined"==typeof a?"Something <strong>bad</strong> happened.":a,0,"#842f15","white",58)}
function restore_options(){var a=$("#email").val();crapi.clone(function(b){"undefined"==typeof b[a]&&(b[a]={});console.log(b);var c=0,e=function(){status("Settings restored.")};$(".option").each(function(){var d=$(this).attr("id");console.log("i am option "+d);d in b[a]||(b[a][d]=d in optionDefaults?optionDefaults[d]:!0,c++);$("#"+d).attr("checked",b[a][d])});"undefined"==typeof b[a].watchlist&&(b[a].watchlist=[]);"undefined"==typeof b[a]["watchlist-queue"]&&(b[a]["watchlist-queue"]=[]);c?(console.log("is missing"),
console.log("setting defaults..."),crapi.update(a,b[a],e)):(console.log("isn't missing"),e())})}
function check_watchlist(a,b){"undefined"==typeof a&&(a=!0);"undefined"==typeof b&&(b=function(){});var c=$("#email").val(),e=[];chrome.storage.local.get(c,function(d){function p(){var p=[];$.each(e,function(a,b){var e=b.link,n=$(b.html),g={},f={};delete b.html;var q=e.match(/user\/.*\/track\//)?!0:!1,m=e.match(/group\/.*/)?!0:!1,h=n.find(".comment .actions").children(".flag, .delete").first();if(h.length){var k;switch(h.attr("class")){case "flag":k=h.attr("onclick");break;case "delete":k=h.attr("href")}"undefined"!=
typeof k&&(g.comment=k.match(/(\d+)/g)[0])}q?(g.title=n.find("#main .title").text(),g.likes=n.find(".stats .likes").text(),g.downloads=n.find(".stats .downloads").text()):m&&(g.title=n.find("#group-info h2").html(),n=n.find("#group-info .info").text(),g.members=n.match(/(\d*) member/)[1],g.following=n.match(/(\d*) follower/)[1]);k=g.title;h=g.likes;m=g.downloads;q=g.members;n=g.following;g=g.comment;k&&(f.title=k,d[c].watchlist[a].title=k);if(h&&b.likes!=h){var r=k=h-b.likes;0<k&&(r="+"+k);d[c].watchlist[a].likes=
h;isNaN(k)||(f.likes=r+" like"+(1<k||-1>k?"s":""))}m&&b.downloads!=m&&(h=m-b.downloads,k="+"+h,d[c].watchlist[a].downloads=m,isNaN(h)||(f.downloads=k+" download"+(1<h?"s":"")));q&&b.members!=q&&(h=m=q-b.members,0<m&&(h="+"+m),d[c].watchlist[a].members=q,isNaN(m)||(f.members=h+" members"+(1<m||-1>m?"s":"")));n&&b.following!=n&&(m=q=n-b.following,0<q&&(m="+"+q),d[c].watchlist[a].following=n,isNaN(q)||(f.following=m+" follower"+(1<q||-1>q?"s":"")));g&&b.comment!=g&&("undefined"!=typeof b.comment&&(f.comment=
"New comment"),d[c].watchlist[a].comment=g);1<Object.keys(f).length&&(f.link=e,p.push(f))});console.log(p);console.debug("final iteration complete!");a?update_storage(c,d[c],b):b()}var f=d[c].watchlist||{},r=f.length,s=0;$.each(f,function(a,b){var c=b.link;$.get("http://soundation.com/"+c,function(c){b.html=c;e[a]=b}).fail(function(){b.fail=!0;e[a]=b}).done(function(){delete b.fail}).always(function(){s++;$("[value='"+c+"']").closest(".wItem").removeClass("load");s==r&&(status("Watchlist updated successfully."),
console.debug("final check complete!"),p())})})})}function update_storage(a,b,c){"undefined"==typeof c&&(c=function(){});var e={};e[a]=b;chrome.storage.local.set(e,c)}function delete_watchlist(a,b){"undefined"==typeof b&&(b=function(){});var c=$("#email").val();chrome.storage.local.get(c,function(e){var d=!1;$.each(e[c].watchlist,function(b,f){return f.link==a?(e[c].watchlist.splice(b,1),d=!0,!1):!0});d&&update_storage(c,e[c],b)})}
function refresh_watchlist(){var a=$("#email").val();chrome.storage.local.get(a,function(b){b=b[a].watchlist||{};var c=$("#watchlist-container");c.html("");$.each(b,function(a,b){var p=_factory("wItem");try{var f=b.link,r=b.title,s=f.match(/user\/.*\/track\//)?!0:!1,t=f.match(/group\/.*/)?!0:!1;s&&p.addClass("track");t&&p.addClass("group");b.fail&&p.addClass("fail");p.find(".label").html(r);p.find(".link").val(f);p.find("button.view").click(function(){window.open("http://soundation.com/"+f,"_blank")});
p.find("button.remove").click(function(){confirm("Click OK to remove '"+r+"' from your watchlist.\n\nYou can always add it again later by using the 'Watch' button on the page.")&&delete_watchlist(f,refresh_watchlist)})}catch(u){console.err(u)}p.appendTo(c)})})}String.prototype.hashCode=function(){var a=0,b,c;if(0==this.length)return a;b=0;for(l=this.length;b<l;b++)c=this.charCodeAt(b),a=(a<<5)-a+c,a|=0;return a};
