...
$("#modation-watchlist .modation-watchlist-item").slideUp(400,function(){$(this).remove();parseWatchlist()})}))):$("#modation-watchlist .wrapper").append(_factory("modation-watchlist-empty"));$("#modation-watchlist .loader").hide()})})}function clear_queue(a,d){crapi.clone(function(c){var b=c[a]["watchlist-queue"][d],f=c[a].watchlist[b.index],e;for(e in b.state)b.state.hasOwnProperty(e)&&(f[e]=b.state[e]);c[a].watchlist[b.index]=f;c[a]["watchlist-queue"].splice(d,1);crapi.update(a,c[a])})}
function purge_queue(a){crapi.clone(function(d){$.each(d[a]["watchlist-queue"],function(c,b){var f=d[a].watchlist[b.index],e;for(e in b.state)b.state.hasOwnProperty(e)&&(f[e]=b.state[e]);d[a].watchlist[b.index]=f});d[a]["watchlist-queue"]=[];crapi.update(a,d[a])})}
