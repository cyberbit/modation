...
Array.prototype.diff=function(a){return this.reduce(function(b,c){-1==a.indexOf(c)&&b.push(c);return b},[])};function handle(a,b,c,d){a=$(a);a.off(b);a.on(b,c)}function getBlob(a,b){var c=new XMLHttpRequest;c.open("GET",a,!0);c.responseType="blob";c.onload=function(a){a=window.URL.createObjectURL(this.response);b(a)};c.send()}
function getDataUri(a,b){var c=new Image;c.onload=function(){var a=document.createElement("canvas");a.width=this.naturalWidth;a.height=this.naturalHeight;a.getContext("2d").drawImage(this,0,0);b(a.toDataURL("image/png").replace(/^data:image\/(png|jpg);base64,/,""));b(a.toDataURL("image/png"))};c.src=a}function localJSON(a,b){if("undefined"==typeof b){var c=localStorage[a];"undefined"!==typeof c&&(c=$.parseJSON(c));return c}""===b?delete localStorage[a]:localStorage[a]=JSON.stringify(b)}
function showView(a,b,c){"undefined"==typeof c&&(c=170);$(a).fadeOut(c);$(b).delay(c).fadeIn(c)}function _factory(a,b){return $(a+" "+b).clone()}crapi=new CrAPI;CrAPI.prototype.manifest=function(){return chrome.runtime.getManifest()};CrAPI.prototype.reload=function(){location.reload()};CrAPI.prototype.debug=crapi.manifest().debug;CrAPI.prototype.lock=function(a){"undefined"==typeof a&&(a="");localStorage.crapi_locked=a};CrAPI.prototype.locked=function(){return localStorage.crapi_locked};
...
e);b.updateAll($.extend(!0,{},d,e),function(){b.clone(c,function(b){console.debug("Result: %s",JSON.stringify(b));console.groupEnd();a()})})})})})})}catch(f){console.error(f),console.groupEnd()}};modapi=new ModAPI;modapi.manifest=crapi.manifest();
var protocol="https://",domain="soundation.com",global={path:{protocol:protocol,domain:domain,home:protocol+domain,cookie:protocol+domain,login:protocol+domain+"?login=yes",feed:protocol+domain+"/feed",api:protocol+"api."+domain+"/me",profile:protocol+domain+"/account/profile",messages:protocol+domain+"/account/messages"},cookie:"_soundation_session",storageModel:{options:{},version:""},optionDefaults:{rememberMe:!1,showAlertsOnTop:!1,userTags:!0,userTagLinks:!1,moveCommentBox:!0,groupFilters:!0,
moveGroupInvites:!0}};function ModAPI(){console.log("%cModAPI%c :: Init","font-weight: bold; color: #f60","");this.DEFAULT_CALLBACK=function(){}}
ModAPI.prototype.login=function(a){function b(a){$.getJSON(global.path.api).fail(function(a,b){console.error("%cModAPI%c :: Unable to connect to Soundation API","font-weight: bold; color: #f60","")}).success(function(b){var e=b.data||b;e.success=b.success;a(e)})}"undefined"==typeof a&&(a=this.DEFAULT_CALLBACK);console.log("%cModAPI%c :: Login","font-weight: bold; color: #f60","");$.get(global.path.home,function(c){c=c.deres();c=$(c);var d=c.find(".user-link").attr("href"),e=c.filter("[name=csrf-token]").attr("content");
