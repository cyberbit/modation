String.prototype.hashCode=function(){var hash=0,i,c;if(this.length==0)return hash;for(i=0,l=this.length;i<l;i++){c=this.charCodeAt(i);hash=(hash<<5)-hash+c;hash|=0}return hash};String.prototype.deres=function(){return this.replace(/img[^>]*>/g,"")};Array.prototype.unique=function(){return this.reduce(function(p,c){if(p.indexOf(c)<0)p.push(c);return p},[])};
function handle(target,event,callback,trigger){if(typeof trigger=="undefined")trigger=false;var $target=$(target);$target.off(event);$target.on(event,callback)}function showView(context,view,fade){if(typeof fade=="undefined")fade=170;$(context).fadeOut(fade);$(view).delay(fade).fadeIn(fade)}crapi=new CrAPI;CrAPI.prototype.manifest=function(){return chrome.runtime.getManifest()};CrAPI.prototype.reload=function(){location.reload()};CrAPI.prototype.debug=crapi.manifest().debug;
CrAPI.prototype.lock=function(state){if(typeof state=="undefined")state="";localStorage.crapi_locked=state};CrAPI.prototype.locked=function(){console.log("lock checked");return localStorage.crapi_locked};
