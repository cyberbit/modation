...
$.get(global.path.feed,function(html){var $html=$(html.deres());var $aside=$html.find("aside");var $notifications=$aside.find(".notifications .notification");var messageLinks=[];var serverNotifs=[];$notifications.find("a").each(function(i,e){var $link=$(e);var href=$link.attr("href");$link.attr("href",global.path.home+href)});$notifications.each(function(i,e){var $notif=$(e);var $actions=$notif.find(".actions").children();var $msg=$notif.clone();$msg.find("time, .clear, .actions").remove();var $links=
$msg.find("a");$msg.find(".author").remove();var id="modation"+$notif.find(".clear").attr("href").match(/(?:\/)(\d+)/)[1];var time=$notif.find("time").text();var from=$notif.find(".author").text();var msg=$msg.text().trim();var link=$links.last().attr("href");var clear=$notif.find(".clear").attr("href");var actions=[];$actions.each(function(i,e){actions.push({title:$(this).text(),link:$(this).attr("href"),data:$(this).data()})});if(link.match(/\/account\/messages\/\d+$/)){if($.inArray(link,messageLinks)!=
-1)return;messageLinks.push(link)}serverNotifs.push(id);var notif={type:"basic",iconUrl:"img/newiconflat128.png",title:from,message:msg,contextMessage:time,buttons:actions.map(function(v){return{title:v.title}})};chrome.notifications.update(id,notif,function(updated){if(!updated)chrome.notifications.create(id,notif);notifData[id]={notif:notif,link:link,clear:clear,actions:actions}})});chrome.notifications.getAll(function(notifs){var localNotifs=Object.keys(notifs);$.each(localNotifs.diff(serverNotifs),
function(i,v){chrome.notifications.clear(v)})});alarmRunning=false})}}catch(e){console.error("Unknown error occurred: %o",e);alarmRunning=false}})})};
