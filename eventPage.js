...
//Check watchlist old version
/*function check_watchlist(email, update) {			
	if (typeof update == "undefined") update = true;
	...
	chrome.storage.local.get(email, function(d) {
		...
		$.each(watchlist, function(i, v) {
			...
			$.get("http://soundation.com/" + link, function() {
				console.log("link " + link + " success");
			}).fail(function() {
				console.log("link " + link + " failed, adjusting wLen");
				wLen--;
			}).always(function() {
				//If last index, run iteration
				if (i == wLen - 1) {
		...
		function _iterate() {
			$.each(watchlist, function(i, v) {
				var wItem = watchlist[i];
				var link = wItem['link'];
				var isTrack = (link.match(/user\/.*\/track\//) ? true : false);
				var isGroup = (link.match(/group\/.*) ? true : false);
				
				//console.log(link + " is track: " + isTrack);
				
				$.get("http://soundation.com/" + link, function(html) {
					var $html = $(html);
					
					var commentActions = $html.find(".comment .actions").children(".flag, .delete").first();
					var commentIDs = [];
					var wNewItem = {};
					...
					commentActions.each(function() {
						...
						commentIDs.push(attr.match(/(\d+)/g)[0]);
					});
					
					wNewItem['comments'] = commentIDs[0];
					
					//alert("comment IDs: " + commentIDs);
					
					//console.log("latest comment has ID " + wNewItem['comment']);
					...
					if (isTrack) {
						...
						//console.log("TRACK :: got track " + wNewItem['title'] + "!");
						//console.log("TRACK :: has " + wNewItem['likes'] + " likes");
						//console.log("TRACK :: downloaded " + wNewItem['downloads'] + " times");
					}
					...
					//Is group
						...
						//console.log("GROUP :: got group " + wNewItem['title'] + "!");
						//console.log("GROUP :: has " + wNewItem['members'] + " members");
						//console.log("GROUP :: " + wNewItem['following'] + " users are following");
					}
					...
					var members = wNewItem['members'];
					var following = wNewItem['following'];
					var comments = wNewItem['comments'];
					
					//console.log("wNewItem:");
					//console.log(wNewItem);
					...
					//Comments hook
					if (comments && wItem['comments'] != comments) {
						//Set comments in storage
						d[email]["watchlist"][i]['comments'] = comments;
					...
					//On last item, update storage
					if (i == wLen - 1) {
						console.log("final iteration complete!");
						if (update) update_storage(email, d[email]);
					}
				}).fail(function() {
					console.log("link " + link + " has an error. unable to update!");
				});
			});
		}
	});
